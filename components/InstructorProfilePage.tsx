...
import { getInstructorById, getInstructorCardTags, getInstructorTaxonomyLabel, instructors } from '../data/instructors';
import { getInstructorBeyondGAById } from '../data/instructorBeyondGA';
import AppFooter from './AppFooter';
import InstructorActionGallery from './InstructorActionGallery';
...
const InstructorProfilePage: React.FC<InstructorProfilePageProps> = ({ instructorId }) => {
  ...
  const t = lang === 'pt'
    ? {
        ...
        notFoundTitle: 'Mestre Arquiteto não encontrado',
        ...
        noticeBody: 'A função de acompanhamento da sua jornada com este Mestre Arquiteto ainda está em construção. Em breve, trilhas, dicas e desafios serão conectados a este perfil.',
    ...
    : {
        ...
        notFoundTitle: 'Master Architect not found',
        ...
        actionTitle: 'In action',
        noticeTitle: 'Journey tracking coming soon',
        noticeBody: 'The journey-tracking feature for this Master Architect is still under construction. Soon, tracks, tips and challenges will be connected to this profile.',
  ...
  if (isExperienceDirector) {
    ...
    t.noticeBody = lang === 'pt'
      ...
      : 'Diana Helena Moreau Fontenelle welcomes, guides and connects people with the different paths and possibilities across Guitar Architect.';
  }
  ...
  const beyondGAItems = beyondGA
    ? [
        ...
        { key: 'otherConnections', label: t.otherConnections, value: beyondGA.otherConnections?.[lang], wide: true },
        { key: 'occupation', label: t.occupation, value: beyondGA.occupation[lang], wide: true },
        { key: 'hobbies', label: t.hobbies, value: beyondGA.hobbies[lang], wide: true },
          ...
          <div className={`overflow-hidden rounded-[40px] border ${cardClass}`}>
            <div className="p-8 pb-6 md:p-12 md:pb-8">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {getInstructorCardTags(instructor).map(tag => (
                  <span
                    key={tag}
            ...
            <div className="p-8 md:p-12">
              <h2 className="mb-3 text-sm font-black uppercase tracking-tight">{instructor.aboutLabel[lang]}</h2>
              <p className={`text-sm md:text-base leading-relaxed ${isLight ? 'text-zinc-700' : 'text-zinc-300'}`}>
              ...
              {instructor.actionGallery?.images.length ? (
                <InstructorActionGallery
                  gallery={instructor.actionGallery}
                  lang={lang}
                  isLight={isLight}
                  title={t.actionTitle}
                />
