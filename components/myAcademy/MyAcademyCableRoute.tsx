...
const MobileCable: React.FC<Omit<MyAcademyCableRouteProps, 'mobile'>> = ({ animate, selectedMomentId, moduleCount, confirmedMomentIds }) => {
  ...
  const endX = Math.min(72, Math.max(8, geometry.finalPoint.x - 4));
  const endY = geometry.finalPoint.y - 10;
  ...
  return (
    <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox={`0 0 100 ${geometry.height}`} preserveAspectRatio="none" aria-hidden="true">
      ...
      <svg data-cable-connector="start" x="2" y="25" width="30" height="26" viewBox="0 0 68 24" preserveAspectRatio="xMinYMid meet">
      ...
      <svg data-cable-connector="end" x={endX} y={endY} width="28" height="24" viewBox="0 0 70 24" preserveAspectRatio="xMaxYMid meet">
