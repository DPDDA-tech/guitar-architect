        ...
        <div className="relative mx-auto max-w-[1100px] py-6">
          ...
          <section className={`overflow-hidden rounded-[36px] border ${panelClass}`}>
            <div className="grid md:grid-cols-[0.85fr_1.15fr]">
              ...
              <div className="flex flex-col justify-center p-7 md:p-12">
                ...
                <blockquote className={`mt-7 border-l-4 border-blue-500 pl-5 text-lg font-semibold italic leading-relaxed ${isLight ? 'text-zinc-700' : 'text-zinc-200'}`}>
                  “{specialist.quote?.[lang]}”
                </blockquote>
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            <section className={`rounded-3xl border p-6 ${panelClass}`}>
              <h2 className="text-sm font-black uppercase tracking-widest text-blue-500">{t.biography}</h2>
              <p className={`mt-4 text-sm leading-7 ${isLight ? 'text-zinc-700' : 'text-zinc-300'}`}>{specialist.biography?.[lang]}</p>
            </section>
            <section className={`rounded-3xl border p-6 ${panelClass}`}>
              <h2 className="text-sm font-black uppercase tracking-widest text-blue-500">{t.philosophy}</h2>
              <p className={`mt-4 text-sm leading-7 ${isLight ? 'text-zinc-700' : 'text-zinc-300'}`}>{specialist.philosophy?.[lang]}</p>
            </section>
          </div>

          <div className="mt-6 grid gap-6 md:grid-cols-[1.1fr_0.9fr]">
            <section className={`rounded-3xl border p-6 ${panelClass}`}>
              <h2 className="text-sm font-black uppercase tracking-widest text-blue-500">{t.areas}</h2>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {specialist.areas?.map(area => <div key={area.pt} className={`rounded-2xl border p-4 text-sm font-semibold ${isLight ? 'border-zinc-200 bg-slate-50' : 'border-zinc-800 bg-zinc-900/60'}`}>{area[lang]}</div>)}
              </div>
            </section>
            <section className={`rounded-3xl border p-6 ${panelClass}`}>
              <h2 className="text-sm font-black uppercase tracking-widest text-blue-500">{t.profile}</h2>
              <dl className="mt-4 space-y-4">
                {profileItems.map(([label, value]) => <div key={label}><dt className={`text-[9px] font-black uppercase tracking-widest ${isLight ? 'text-zinc-400' : 'text-zinc-500'}`}>{label}</dt><dd className="mt-1 text-sm font-semibold">{value}</dd></div>)}
              </dl>
            </section>
          </div>
          ...
          <div className={`mt-6 rounded-3xl border p-6 text-sm leading-relaxed ${isLight ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-amber-900/60 bg-amber-950/30 text-amber-200'}`}>
            <p>{t.notice}</p>
            <p className="mt-3 text-xs opacity-75">{t.fictional}</p>
          </div>
        </div>
      </main>
