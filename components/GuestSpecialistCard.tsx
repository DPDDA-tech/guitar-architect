...
const GuestSpecialistCard: React.FC<GuestSpecialistCardProps> = ({ guest, isLight, lang }) => {
  ...
  if (!isActive) {
    return (
      <article className={`flex min-h-[430px] flex-col overflow-hidden rounded-3xl border border-dashed opacity-75 ${panelClass}`}>
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <h3 className={`mt-3 text-sm font-black uppercase tracking-tight ${isLight ? 'text-zinc-800' : 'text-white'}`}>
            {guest.specialty[lang]}
          </h3>
          <p className={`mt-2 text-xs leading-relaxed ${isLight ? 'text-zinc-600' : 'text-zinc-400'}`}>
            {guest.shortDescription[lang]}
          </p>
        </div>
      </article>
    );
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <p className={`mt-2 text-[9px] italic opacity-60 ${isLight ? 'text-zinc-400' : 'text-zinc-500'}`}>
          {lang === 'pt' ? 'Personagem fictícia · IA' : 'Fictional character · AI'}
        </p>
        <span className="mt-4 text-[10px] font-black uppercase tracking-widest text-blue-500">
