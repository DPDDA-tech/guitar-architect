...
type AppLang = 'pt' | 'en';
type FilterValue = 'all' | InstructorCategory;
...
const InstructorsGalleryPage: React.FC = () => {
  ...
  const filteredInstructors = useMemo(
    () => (filter === 'all' ? instructors : instructors.filter(instructor => instructor.categories.includes(filter))),
    [filter],
  );
  ...
  const t = lang === 'pt'
    ? {
        ...
        subtitle: 'Conheça os personagens virtuais que orientam sua jornada no Guitar Architect. Cada arquiteto musical representa uma forma de aprender, praticar e entender música.',
    ...
    : {
        ...
        subtitle: 'Meet the virtual characters who guide your journey in Guitar Architect. Each music architect represents a way of learning, practicing and understanding music.',
    ...
    <>
      <div className={`relative min-h-screen p-6 md:p-12 ${isLight ? 'bg-slate-50 text-zinc-900' : 'bg-zinc-950 text-white'}`}>
        ...
        <div className="relative mx-auto max-w-[1200px] py-6">
          ...
          <div className="mb-8">
            <p className={`mb-3 text-center text-[10px] font-black uppercase tracking-widest ${isLight ? 'text-zinc-400' : 'text-zinc-500'}`}>
              {t.filterLabel}
            </p>
