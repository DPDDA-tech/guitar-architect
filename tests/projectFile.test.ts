...
import { buildProjectExportFile, parseProjectFile, ProjectFilePayload } from '../utils/projectFile';
...
describe('project file export/import', () => {
  ...
  it('builds a valid export payload', () => {
    const payload = buildProjectExportFile(project, {
    ...
    expect(payload.appVersion).toBe('1.8.1');
    expect(payload.project.name).toBe('Project One');
    expect(payload.project.theme).toBe('dark');
    expect(payload.project.lang).toBe('en');
    expect(payload.project.userLogo).toBe('https://example.com/logo.png');
